...
if [ $number -ge 104 ]; then  #ARC104以降(6問時代)
  ...
  if [ $number -eq 120 ]; then
    ...
    mv "$filename" "$name"  #そこへ作成したファイルを移動
    cd "$name"  #そこへ移動
    oj d "${url_base}f2"  #テストケースを取得
    cd ../  #arcxxxディレクトリへ戻る
  fi


elif [ $number -ge 58 ]; then  ##ARC58以降(4問時代, c-f)
  for i in c d e f
    do
    name="${contest}${str_number}_${i}"  #ディレクトリの名前
    filename="${name}.cpp"  #ファイルの名前
  ...
  for i in a b c d
    ...
    mv "$filename" "$name"  #そこへ作成したファイルを移動
    cd "$name"  #そこへ移動
    if [ $number -ge 35 ]; then  #35回以上はURLは.._a
      oj d "${url_base}${i}"  #テストケースを取得
    else  #34回未満はURLは.._1
      ar=""  #c-f時代はURLはc->a, d->b..となっている
      if [ $i = a ]; then
